...
xml.search('anime').each_with_index do |anime, count|
  ...
  titles.each do |title|
    ...
    anime_ens.each do |anime_en|
      if not anime_en.nil? and (anime_en.content.downcase.include? title.name.downcase or
        title.name.downcase.include? anime_en.content.downcase)
      #if anime_en.content.casecmp(title.name) == 1 or anime_title.content.casecmp(title.name) == 0
      #if anime_en.content.any?{ |s| s.casecmp(title.name)==0 }
        output.root.add_child anime
        puts "Added an en element: #{anime_en.content}"
        match_count += 1
        hash[title.name] = true
        matched = true
        break
      end
    end
    ...
    anime_mains.each do |anime_main|
      if not anime_main.nil? and (anime_main.content.downcase.include? title.name.downcase or
        title.name.downcase.include? anime_main.content.downcase)
        output.root.add_child anime
    ...
    anime_officials.each do |anime_official|
      if not anime_official.nil? and (anime_official.content.downcase.include? title.name.downcase or
        title.name.downcase.include? anime_official.content.downcase)
        output.root.add_child anime
        hash[title.name] = true
