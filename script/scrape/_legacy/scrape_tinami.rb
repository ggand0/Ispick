...
module Scrape
  class Tinami < Client
    ...
    def initialize(logger=nil, limit=20)
      ...
      if logger.nil?
        self.logger = Logger.new('log/scrape_tinami_cron.log')
      else
        self.logger = logger
      end
    ...
    def scrape(interval=60)
      ...
      scrape_target_words('Scrape::Tinami', interval)
    end


    # キーワードによる抽出処理を行う
    # @param [TargetWord]
    def scrape_target_word(target_word, english=false)
      @limit = 10
      @logger.info "Extracting #{@limit} images from: #{ROOT_URL}"

      result = scrape_using_api(target_word, true, false, english)
    ...
    def scrape_using_api(target_word, validation=true, logging=false)
      ...
      search.contents.content.each_with_index do |image, count|
        ...
        # illustまたはmangaのみを抽出
        if content.content.type != 'illust' then #&& content.content.type != 'manga' then
          skipped += 1
          next
        end
